	...
	<div class="d-flex flex-column flex-column-fluid">
		...
		<div id="kt_app_content" class="app-content flex-column-fluid">
			<div id="kt_app_content_container" class="app-container container-fluid ">
			    <form method="POST" action="{{route('permission.store')}}" id="permission_form" enctype="multipart/form-data">
			        @csrf
				    <div class="card">
    					<div class="card-body">
                                    ...
                                    <div class="row">
                                        <div class="col-md-4 col-sm-12">
                                            <div class="mb-5 fv-row">
                                                ...
                                                <select class="form-select btn-sm permission_input" name="name" data-control="select2"  data-hide-search="true">
                                                    <option value="access" {{old('name') == "access" ? 'selected' : ''}}>Access</option>
                                                    <option value="create" {{old('name') == "create" ? 'selected' : ''}}>Create</option>
                                        ...
                                        <div class="col-md-4 col-sm-12">
                                            <div class="mb-5 fv-row">
                                                ...
                                                <select class="form-select btn-sm permission_input" name="module" data-control="select2"  data-hide-search="true">
                                                    <option value="borrow" {{old('module') == "borrow" ? 'selected' : ''}}>Borrow</option>
                                                    <option value="fund" {{old('module') == "fund" ? 'selected' : ''}}>Fund</option>
                                                    <option value="payment" {{old('module') == "payment" ? 'selected' : ''}}>Payment</option>
                                                    <option value="client" {{old('module') == "client" ? 'selected' : ''}}>Client</option>
                                                    <option value="staff" {{old('module') == "staff" ? 'selected' : ''}}>Staff</option>
                                                    <option value="administrator" {{old('module') == "administrator" ? 'selected' : ''}}>Administrator</option>
                                                    <option value="report" {{old('module') == "report" ? 'selected' : ''}}>Report</option>
                                                    <option value="roles-and-permissions" {{old('module') == "roles-and-permissions" ? 'selected' : ''}}>Roles And Permissions</option>
                                                </select>
                                                @error("module")
                                                    <div class="text text-danger">{{ $message }}</div>
                                                @enderror
